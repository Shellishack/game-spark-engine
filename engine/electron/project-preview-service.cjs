...
async function copyPreviewRuntime(buildRoot) {
  const source = path.join(appRoot, "web", "runtime", "preview-editor-controls.js");
  const target = path.join(buildRoot, "preview-editor-controls.js");
...
function injectPreviewEditorControls(html) {
  const withoutLegacyInline = removeLegacyPreviewEditorControls(html);
...
function removeLegacyPreviewEditorControls(html) {
  return html.replace(/\s*<script>\s*([\s\S]*?)<\/script>/gi, (match, scriptBody) => {
    if (!scriptBody.includes("GAME_SPARK_PREVIEW_CONTROL")) return match;
    if (!scriptBody.includes("BABYLON.EngineStore") && !scriptBody.includes("event.preventDefault")) return match;
    return "";
  });
}
